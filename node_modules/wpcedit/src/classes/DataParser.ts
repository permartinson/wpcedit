// ####### TODO:
// Fix bugs:
// - Table address not working (maybe just document the error)
// Cleanup:
// - Write readme
// - Add GitHub link in "about"-section
// - Add descriptions to WPCEdit.ts file
// - Use global CSS classes for standard text styles
// - Run lint tool (at least for wpcedit.ts)
// - Remove this todo from the file
// Wishlist:
// - Change state when holding a file over the file drop


// DONE:
// Custom image for settings page
// - Add comments to code
// - Remove uncommented code
// - Bug when exporting some sprites as BMP (put in fullframes + document the error)
// - Remember sprite offsets between frames
// - Cannot generate pngs from other displays than the plasma style
// - Re-export symbols with better precision
// - Search and remove all console.log()
// - Use consistent naming: Planes should always be planes and images should always be images
// - Rename "DirectionalButton" to "MinimalButton"
// Create App icon
// - Improve texts (especially settings explanations)
// - Improve pagination block
// - Make the "next" and "prev" buttons step x2 in merge panel when blend is checked
// - Route error when entering /fullframe
// Add PWA framework
// Add WPC Edit to title tag
// Get a max full frame index
// - File drop not working in Firefox
// - Test in Firefox and Safari
// - Test with other roms
// Finalize raw data export
// - Merge WpcEditStore and AppState
// - Improve layout for settings view
// - Remove all web worker code
// Add responsive CSS
// - Limit file name size in header
// Add more error messages when load fails
// - It is possible to load a non-rom file
// - Putting sprites in fullframes sometimes loses the last column
// - Sprites list does not fit within the grid
// - Some graphics are not rendered when lazy-loading
// Remember blend setting when stepping between planes
// Synchronize stepping of merge planes when blend is active
// - Cancel dedicated worker after it was used
// Add addresses and indexes to tables and modal headers
// - Sub pages are always redirecting to root when reloading
// Implement XOR flags/bytes
// Add ability to export graphics:
// - From rendered canvas
// - As raw data
// - As 1bit .bmp 
// Add settings view and implement settings:
// - Preferred render option
// - Option to make auto-sequencing optional
// - DMDs per page
// - Reset warning messages
// Implement pagination
// Add warning messages when:
// - User wants to unload ROM (option to not show)
// - User is loading a ROM (first X times)
// - Something went wrong while loading the ROM
...
export class DataParser {
    ...
    for (Page = 0; Page < ROM.totalPages; Page++) {
      for (PageByteIdx = 0; PageByteIdx < WPC.PageLength; PageByteIdx++) {
        ...
        switch (ROM.byteAtAddr(Ptr++) & 0xFF) {
          ...
          // Search for: BE xx xx 3A 58 3A D6 yy 34 04 BD zz zz F6 ww ww, WPC Table address address is xxxx in non-banked ROM
          // Search for: BE xx xx 3A 58 3A D6 yy 34 04 F6 zz zz F6 ww ww, WPC Table address address is xxxx in non-banked ROM
          if (PageByteIdx >= WPC.PageLength - 16) { // don't try to read out of bounds, going to read up to 16 bytes after Ptr
            break;
            //return -1;
          }
          if (
  ...
  public static getLastImageIndex(pImageIndex:number,TableIndex: number) : [number, number[]]  {
    let ImageIndex = 0;
    let hit = 0;
    let counter = 0;
    let imageIndeces = [0];
...
public static getPrevImageIndex(pImageIndex:number, TableIndex:number) {
  ...
  public static extractWPCAddrAndPageOfImageTable(TableIndex:number) {
    ...
    romAddr = DataParser.getAddrToWPCAddressOfVariableSizedImageTable(
      TableIndex
    );
    if (romAddr == -1) {
      return [-1,-1];
  ...
  private static getAddrToWPCAddressOfVariableSizedImageTable(TableIndex: number) {
    let romAddr: number;
...
public static getROMAddressOfVariableSizedImageIndex(TableIndex:number, ImageIndex:number) {
	let Addr:number;
	let Page:number;
	let Ptr:number;
	let TableHeight:number;
	let TableSpacing:number;  // not sure at this point, 0x01 follows TableHeight
	let ImageIndexMin:number, ImageIndexMax:number, ImageNum:number, ImageFound:number;
