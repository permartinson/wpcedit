...
public static decodeNextIndex(count: number, dataType:number) {
  ...
  private static incrementVariableSizedImageIndex(pTableIndex:number, pImageIndex:number) {
...
private static decrementVariableSizedImageIndex(pTableIndex:number, pImageIndex:number) {
	let tmpImageIndex:number;
...
public static decodePreviousIndex(count: number, dataType:number) {
  ...
  public static decodeVariableSizedImageIndexToPlane(TableIndex:number, ImageIndex:number):[number,Defines.DMDPlanes] {
	let DataPtr:number;
	let Addr:number;
...
private static decodeVariableSizedImage(Source:number, pPlanes:Defines.DMDPlanes, TableIndex:number):[number,Defines.DMDPlanes] {
...
private static decodeVariableSizedImageToBits(SourcePtr: number, Dest:Uint8Array, ImageHeight:number, ImageWidth:number, Centered=false):[number,Uint8Array,number] {
	let ch: number;
	let WriteCounter = 0;
	let i,j:number;
  let DestPtr = 0;
...
private static decodeVariableSizedImageIndex_NoHeader(SourcePtr:number, pPlanes:Defines.DMDPlanes, TableHeight:number) {
...
private static decodeVariableSizedImageIndex_Header(SourcePtr:number, pPlanes:Defines.DMDPlanes, TableHeight:number, TableIndex:number) {
	let DestPlane0 = pPlanes.Plane0.Plane_Data;
  ...
  private static decodeFullFrameGraphicImage(Source:number, pPlane:Defines.DMDPlane, SkipDecoding:boolean) {
    ...
    if(SkipDecoding) { // If we just want to get the type, let's skip all the decoding.
      return pPlane;
    }
 ...
 private static decode_08or09(SourcePtr: number, Type: number) {
  ...
  private static writeNext8BitValue(WriteCounterPtr: number, Dest: Uint8Array, DestPtr: number, ch: number, Type: number): [number,Uint8Array,number] {
    return DmdDecoder.writeNext8BitValueAnySize(WriteCounterPtr, Dest, DestPtr, ch, Type, WPC.DmdCols, WPC.DmdRows);
  }

    private static writeNext8BitValueAnySize(WriteCounterPtr: number, Dest: Uint8Array, DestPtr: number, ch: number, Type: number, cols: number, rows: number): [number,Uint8Array,number] {
  ...
  private static readNext8BitValue(Header: Defines.ImageHeader, SourcePtr: number) {
